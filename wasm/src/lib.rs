...
pub fn encrypt(text: &str, password: &str, salt: &str, iterations: u32) -> JsValue {
    ...
    let ciphertext = match crypto::encrypt("AES-256-GCM", text, password, salt, iterations) {
...
pub fn decrypt(hex: &str, password: &str, salt: &str, iterations: u32) -> JsValue {
    ...
    let plaintext = match crypto::decrypt("AES-256-GCM", hex, password, salt, iterations) {
